...
# Encoding default para este script é WINDOWS-1252
...
# Redução da sociomatriz (Top 100 artistas mais "Pop")
...
Compute_GLIs <- function(g, use.loops, has.directions, has.weights) {
  ...
  Centralization_Index <- function(g) {
  ...
  list("Graph_Stats" = Graph_Stats(g), "Centralization_Index" = Centralization_Index(g)) %>% return()
  
}
Compute_GLIs <- cmpfun(Compute_GLIs)

## Resultados
Billboard.GLIs <- Compute_GLIs(Billboard.Graph, use.loops = F, has.directions = F, has.weights = T)
...
######### Análises #########
#
# Os algoritmos para detecção de comunidades retornam resultados aparentemente contraditórios :
# alguns subdividem "de mais" a rede (que é o caso com o método Fast Greedy e Leading Eigenvector)
# enquanto outros subdividem menos a rede (como é o caso do Random Walk).
#
...
#
# A confusão se extende mais ainda pois aparentemente, o Grafo não aparenta ser centralizado
# em torno dos vértices com os maiores índices de centraldiade de proximidade. Este é, no entanto,
# altamente centralizado em torno dos vértices com alto valor de centralidade de Grau, Autovetor e Kleinberg.
# (Note como Drake, Ariana Grande estão de fato misturados no centro).
