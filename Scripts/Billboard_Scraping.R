...
## Função que realiza desde a scoleta até montagem da sociomatriz 
Billboard_Scrap <- function() {
  ...
  rio::export(Social_Matrix, paste0(Beggining.Date,"_Billboard_SocialMatrix.csv"), format = ",")
  
  # Sinaliza término
  return(paste0("Fim. Sociomatriz montada contêm dados dos últimos ",length(Months.To_Pull)," meses."))
  
}
Billboard_Scrap <- cmpfun(Billboard_Scrap)
